...
class wiimote(Structure):
    _fields_ = [('unid', c_int),
                ...
                ('event_cb', c_void_p),
                ('dis_cb', c_void_p),
                ('ctrl_status_cb', c_void_p),
                ('handshake_state', c_byte), # int?
                ('read_req', c_void_p),
                ('accel_calib', accel),
                ('exp', expansion),
                ('accel', vec3b),
...
def init(nwiimotes, unids, event_cb, status_cb, dis_cb):
    ...
    find = wapi.find
    connect = wapi.connect
    set_leds = wapi.set_leds
    rumble = wapi.rumble
    status = wapi.status
    poll = wapi.poll
    disconnect = wapi.disconnect
    motion_sensing = wapi.motion_sensing
    set_ir = wapi.set_ir
    toggle_rumble = wapi.toggle_rumble
    set_ir_vres = wapi.set_ir_vres
    set_ir_position = wapi.set_ir_position
    set_aspect_ratio = wapi.set_aspect_ratio
    set_orient_threshold = wapi.set_orient_threshold
    set_flags = wapi.set_flags
    
    # finally initialize wiiuse
    ids = (c_int*nwiimotes)(*unids)
    
    t = wapi.init(nwiimotes, ids, 
                  event_cb_t(event_cb), 
                  ctrl_status_cb_t(status_cb), 
                  dis_cb_t(dis_cb))
    return t
